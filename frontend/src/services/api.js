...
API.interceptors.request.use((config) => {
  ...
  if (token && !skipStaffToken) {
    ...
    try {
      const raw =
        sessionStorage.getItem('eljays-auth') ||
        localStorage.getItem('eljays-auth');
...
// ADMIN — BLOG (AI draft + uploads)
export const adminBlogAPI = {
  aiGenerate: (data) => API.post('/admin/blog/ai-generate', data, { timeout: 120000 }),
};
...
export const adminAiAPI = {
  analyzeImage: (file) => {
    ...
    return API.post('/admin/upload/analyze', form, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 120000,
    });
  },
