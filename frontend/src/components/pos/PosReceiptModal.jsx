...
const PosReceiptModal = ({ sale, sellerName, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-utility-gray rounded-xl p-6 max-w-md w-full text-secondary">
        <h2 className="text-xl font-bold">Sale complete</h2>
        <p className="text-sm text-secondary/50 mt-1">{sale.receipt_number}</p>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={downloadPdf} className="flex-1 bg-accent rounded-lg py-2 font-medium">Download PDF</button>
          <button onClick={onClose} className="flex-1 bg-utility-gray/30 rounded-lg py-2">Close</button>
        </div>
      </div>
    </div>
  );
