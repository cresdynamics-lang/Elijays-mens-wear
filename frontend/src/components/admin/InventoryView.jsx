...
const MOVEMENT_LABELS = {
 ...
 sales: { label: 'Sales', color: 'text-red-600 bg-red-400/10' },
 stock_in: { label: 'Stock In', color: 'text-green-600 bg-green-400/10' },
...
const EmptyRow = ({ cols, msg }) => (
 <tr>
 <td colSpan={cols} className="px-6 py-12 text-center text-secondary/60 text-sm">{msg}</td>
 </tr>
);
...
const ShopsPanel = ({ shops, onRefresh }) => {
 ...
 <h5 className="text-sm font-black text-secondary/70">Shops / Branches</h5>
 ...
 className="bg-primary border border-utility-gray/60 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
 ...
 className="bg-utility-gray border border-utility-gray/60 rounded-xl px-4 py-3 text-sm text-secondary placeholder:text-secondary/70 outline-none focus:border-accent-500/40" />
 ))}
 ...
 <button onClick={() => setShowForm(false)} className="px-4 py-3 bg-utility-gray text-accent-500 rounded-xl text-[10px] font-black ">
 Cancel
 </button>
 </div>
 </motion.div>
 ...
 {shops.map(shop => (
 <div key={shop.id} className="bg-primary border border-utility-gray/60 rounded-2xl p-5 flex items-start justify-between gap-3">
 <div className="flex items-center gap-3 min-w-0">
 <div className="w-10 h-10 rounded-xl bg-accent-600/10 border border-utility-gray/60 flex items-center justify-center shrink-0">
 <Building2 size={18} className="text-accent-500" />
 </div>
 <div className="min-w-0">
 <p className="text-sm font-black text-secondary truncate">{shop.name}</p>
 <p className="text-[10px] text-secondary/60 ">{shop.code}</p>
 {shop.address && <p className="text-[10px] text-secondary/70 truncate">{shop.address}</p>}
 </div>
 </div>
 <button onClick={() => handleDelete(shop.id)} className="text-red-600 hover:text-red-600 transition-colors shrink-0">
 <X size={16} />
 </button>
 </div>
 ))}
 {!shops.length && (
 <div className="col-span-3 py-10 text-center text-secondary/70 text-sm">No shops yet. Add one to start tracking inventory.</div>
 )}
 </div>
 </div>
 );
...
const OpeningStockModal = ({ shops, onClose, onDone }) => {
 ...
 <button onClick={onClose} className="px-5 py-3 bg-utility-gray text-accent-500 rounded-xl text-[10px] font-black ">Cancel</button>
...
const TransferModal = ({ shops, onClose, onDone }) => {
 ...
 <div className="flex items-center justify-between">
 <p className="text-[10px] font-black text-secondary/50">Items</p>
 <button onClick={addItem} className="flex items-center gap-1 text-accent-500 text-[10px] font-black ">
 <Plus size={12} /> Add Item
 </button>
 </div>
 {items.map((item, idx) => (
 <div key={idx} className="grid grid-cols-[1fr_auto_auto] gap-3 items-center bg-primary border border-accent-500/5 rounded-xl p-3">
 ...
 className="w-20 bg-utility-gray border border-utility-gray/60 rounded-xl px-3 py-3 text-sm text-secondary outline-none focus:border-accent-500/40 text-center" />
 {items.length > 1 && (
 <button onClick={() => removeItem(idx)} className="text-red-600 hover:text-red-600"><X size={16} /></button>
 )}
 </div>
 ))}
 </div>
...
const StockSummaryTable = ({ shops }) => {
 ...
 period === p ? 'bg-accent-600 text-base-950 border-accent-600' : 'bg-utility-gray text-accent/80 border-utility-gray/60 hover:border-accent-500/30'
 }`}>{p}</button>
 ))}
 </div>
 <select value={shopFilter} onChange={e => setShopFilter(e.target.value)}
 className="bg-utility-gray border border-utility-gray/60 rounded-xl px-4 py-2 text-sm text-secondary outline-none cursor-pointer">
 <option value="">All Shops</option>
 {shops.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
 </select>
 <button onClick={fetchSummary} className="p-2 text-secondary/50 hover:text-accent-500 bg-utility-gray border border-utility-gray/60 rounded-xl transition-colors">
 <RefreshCw size={16} />
 </button>
 <span className="text-[10px] text-secondary/70">{periodLabels[period]}</span>
 </div>

 <div className="bg-utility-gray border border-utility-gray/60 rounded-2xl overflow-hidden">
 {loading ? <Spinner /> : (
 ...
 <tr key={i} className="hover:bg-utility-gray transition-colors">
 <td className="px-5 py-4 text-sm font-bold text-secondary">{row.product_name}</td>
 <td className="px-5 py-4 text-xs text-secondary/70">{row.variant_label || '—'}</td>
 <td className="px-5 py-4">
 <span className="text-[10px] font-black text-accent-500 bg-accent-500/10 px-2 py-1 rounded-lg">{row.shop_code}</span>
 </td>
 <td className="px-5 py-4 text-right text-secondary font-bold">{row.opening_stock}</td>
 <td className="px-5 py-4 text-right text-green-600 font-bold">+{row.stock_in}</td>
 <td className="px-5 py-4 text-right text-orange-400 font-bold">-{row.stock_out}</td>
 <td className="px-5 py-4 text-right text-red-600 font-bold">-{row.sales}</td>
 <td className="px-5 py-4 text-right">
 <span className={`font-black text-sm ${parseInt(row.closing_stock) <= 5 ? 'text-red-600' : 'text-secondary'}`}>
 {row.closing_stock ?? '—'}
 </span>
 </td>
 </tr>
...
const CurrentStockPanel = ({ shops }) => {
 ...
 <button onClick={fetchData} className="p-2 text-secondary/50 hover:text-accent-500 bg-utility-gray border border-utility-gray/60 rounded-xl transition-colors">
 <RefreshCw size={16} />
 </button>
 </div>
 <div className="bg-utility-gray border border-utility-gray/60 rounded-2xl overflow-hidden">
 ...
 {row.thumbnail && <img src={row.thumbnail} alt={row.product_name} className="w-8 h-8 rounded-lg object-cover border border-utility-gray/60" />}
 <span className="text-sm font-bold text-secondary">{row.product_name}</span>
 </div>
 </td>
 <td className="px-5 py-4 text-xs text-secondary/70">{row.variant_label || '—'}</td>
 <td className="px-5 py-4">
 ...
 <td className="px-5 py-4 text-right">
 <span className={`text-base font-black ${parseInt(row.current_stock) <= 5 ? 'text-red-600' : parseInt(row.current_stock) <= 15 ? 'text-amber-600' : 'text-green-600'}`}>
 {row.current_stock}
 </span>
 </td>
 </tr>
...
const MovementHistory = ({ shops }) => {
 ...
 <tr key={row.id} className="hover:bg-utility-gray transition-colors">
 <td className="px-5 py-4 text-xs text-secondary/50">{new Date(row.created_at).toLocaleString()}</td>
 ...
 <td className="px-5 py-4 text-right font-black text-secondary">{row.quantity}</td>
 <td className="px-5 py-4 text-right font-black text-accent-400">{row.closing_stock}</td>
 <td className="px-5 py-4 text-xs text-secondary/60 max-w-[160px] truncate">{row.description || '—'}</td>
 </tr>
...
const TransferHistory = () => {
 ...
 {data.length ? data.map(row => (
 <tr key={row.id} className="hover:bg-utility-gray transition-colors">
 <td className="px-5 py-4 text-xs text-secondary/50">{new Date(row.created_at).toLocaleDateString()}</td>
 <td className="px-5 py-4 text-sm font-bold text-secondary">{row.from_shop_name}</td>
 <td className="px-5 py-4 text-sm font-bold text-secondary">{row.to_shop_name}</td>
 <td className="px-5 py-4 text-right font-black text-accent-400">{row.total_quantity}</td>
 <td className="px-5 py-4 text-right text-secondary/70">{row.item_count}</td>
 <td className="px-5 py-4">
 <span className="text-[10px] font-black text-green-600 bg-green-400/10 px-2 py-1 rounded-lg">{row.status}</span>
 </td>
 </tr>
...
const ModalWrap = ({ title, onClose, children, wide }) => (
 <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-primary backdrop-blur-sm">
 <div className={`bg-utility-gray border border-utility-gray/60 rounded-3xl p-6 w-full ${wide ? 'max-w-3xl' : 'max-w-lg'} max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl`}>
 <div className="flex items-center justify-between mb-6">
 <h4 className="text-lg font-serif font-bold text-secondary">{title}</h4>
 <button onClick={onClose} className="text-secondary/60 hover:text-accent-500"><X size={20} /></button>
 </div>
 {children}
 </div>
 </div>
...
const SelectField = ({ label, value, onChange, children }) => (
 <div>
 {label && <label className="block text-[10px] font-black text-secondary/50 mb-1">{label}</label>}
 <select value={value} onChange={e => onChange(e.target.value)}
 className="w-full bg-utility-gray border border-utility-gray/60 rounded-xl px-4 py-3 text-sm text-accent-200 outline-none focus:border-accent-500/40 cursor-pointer">
 {children}
 </select>
 </div>
);
...
const InputField = ({ label, value, onChange, type = 'text', placeholder }) => (
 ...
 className="w-full bg-utility-gray border border-utility-gray/60 rounded-xl px-4 py-3 text-sm text-secondary placeholder:text-secondary/70 outline-none focus:border-accent-500/40" />
 </div>
);
...
const InventoryView = () => {
 ...
 <div>
 <span className="text-[10px] font-black tracking-[0.3em] text-secondary/60">Multi-Shop · Real-Time</span>
 <h3 className="text-2xl font-serif font-bold text-secondary mt-1">Inventory Management</h3>
 </div>
 ...
 { id: 'sales', label: 'Record Sales', icon: ShoppingCart, color: 'bg-red-500/20 text-red-600 border-red-500/20 hover:bg-red-500/30' },
 ...
 { label: 'Active Shops', value: shops.filter(s => s.is_active !== false).length, icon: CheckCircle2, color: 'text-green-600' },
 { label: 'Low Stock', value: '—', icon: AlertCircle, color: 'text-red-600' },
 ].map(s => (
 <div key={s.label} className="bg-utility-gray border border-utility-gray/60 rounded-2xl p-4 flex items-center gap-4">
 <div className={`${s.color} opacity-80`}><s.icon size={22} /></div>
 <div>
 <p className="text-[10px] font-black text-secondary/60">{s.label}</p>
 <p className="text-xl font-serif font-bold text-secondary">{loading ? '—' : s.value}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 tab === t.id ? 'bg-accent-600 text-base-950 border-accent-600' : 'bg-utility-gray text-accent/80 border-utility-gray/60 hover:border-accent-500/30'
 }`}>
