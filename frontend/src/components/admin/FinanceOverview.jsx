...
const FinanceOverview = () => {
 ...
 <div>
 <span className="text-[10px] font-black tracking-[0.3em] text-secondary/60">Shop + online — one view</span>
 <h3 className="text-3xl font-serif font-bold text-secondary mt-2">Finance</h3>
 </div>
 ...
 : 'bg-utility-gray text-accent border-utility-gray/60 hover:border-accent-500/40'
 }`}
 >
 {p}
 </button>
 ...
 type="button"
 onClick={card.action}
 className="text-left bg-utility-gray border border-utility-gray/60 rounded-2xl p-6 backdrop-blur-sm transition-all hover:border-accent-500/40 disabled:hover:border-utility-gray/60"
 disabled={!card.action}
 >
 <div className="flex items-center justify-between mb-6">
 <div className="w-11 h-11 rounded-xl bg-accent-600/10 border border-utility-gray/60 flex items-center justify-center text-accent-500">
 <card.icon size={20} />
 </div>
 <span className="text-[10px] text-green-600 font-black ">Live</span>
 </div>
 <p className="text-[10px] tracking-[0.2em] text-secondary/60 font-black">{card.label}</p>
 <h4 className="text-2xl font-serif font-bold text-secondary mt-2">{card.value}</h4>
 <p className="text-xs text-secondary/60 mt-2">{card.detail}</p>
 </button>
 ))}
 </div>

 {dailyStockRows.length > 0 && (
 <div className="bg-utility-gray border border-utility-gray/60 rounded-2xl overflow-hidden backdrop-blur-sm">
 <div className="p-6 border-b border-utility-gray/60 flex items-center justify-between">
 <h4 className="font-serif font-bold text-xl text-secondary">Opening &amp; Closing Stock (today)</h4>
 <span className="text-[10px] text-secondary/60">By category — from POS inventory</span>
 </div>
 <div className="overflow-x-auto">
 <table className="w-full text-left text-sm">
 <thead className="bg-utility-gray text-secondary/50 text-xs ">
 <tr>
 ...
 <tr key={row.name || row.category} className="border-t border-utility-gray/60">
 <td className="p-3 text-secondary">{row.name || row.category}</td>
 <td className="p-3 text-center tabular-nums">{row.opening_qty ?? 0}</td>
 <td className="p-3 text-center tabular-nums text-red-600">{row.sales_qty ?? 0}</td>
 <td className="p-3 text-center tabular-nums text-green-600">{row.stock_in_qty ?? 0}</td>
 ...
 <div className="xl:col-span-2 bg-utility-gray border border-utility-gray/60 rounded-2xl p-6 backdrop-blur-sm">
 <div className="flex items-center justify-between mb-8">
 <h4 className="font-serif font-bold text-xl text-secondary">Revenue Graph</h4>
 <span className="text-[10px] text-secondary/60">{getPeriodLabel(period)}</span>
 </div>
 <div className="h-72 flex items-end gap-3">
 {chartData.map((item) => (
 <div key={item.label} className="flex-1 h-full flex flex-col justify-end gap-3">
 <div className="text-[10px] text-secondary/50 text-center">{formatMoney(item.total).replace('KSh ', '')}</div>
 ...
 <div className="text-[10px] font-black text-secondary/60 text-center">{item.label}</div>
 </div>
 ))}
 </div>
 </div>

 <div className="bg-utility-gray border border-utility-gray/60 rounded-2xl p-6 backdrop-blur-sm">
 <h4 className="font-serif font-bold text-xl text-secondary mb-6">Top Movers</h4>
 <div className="space-y-4">
 {(soldProducts.length ? soldProducts : topProducts).slice(0, 6).map((item, index) => (
 <div key={item.id || item.name} className="flex items-center justify-between p-4 rounded-xl bg-primary border border-accent-500/5">
 <div>
 <p className="text-sm font-bold text-secondary">{item.name}</p>
 <p className="text-[10px] text-secondary/60">Rank #{index + 1}</p>
 </div>
 ...
 <div className="bg-utility-gray border border-utility-gray/60 rounded-2xl overflow-hidden backdrop-blur-sm">
 <div className="p-6 border-b border-utility-gray/60 flex items-center justify-between">
 <h4 className="font-serif font-bold text-xl text-secondary">Products Sold - {getPeriodLabel(period)}</h4>
 <span className="text-[10px] text-secondary/60">{soldProducts.length} products</span>
 </div>
 <div className="overflow-x-auto">
 ...
 <tr key={item.id} className="hover:bg-utility-gray transition-colors">
 <td className="px-6 py-4 text-sm font-bold text-secondary">{item.name}</td>
 <td className="px-6 py-4 text-accent-500 font-black">{item.quantity}</td>
 <td className="px-6 py-4 text-secondary">{formatMoney(item.revenue)}</td>
 <td className="px-6 py-4 text-green-600">{formatMoney(item.profit)}</td>
 </tr>
 )) : (
 <tr>
 <td colSpan="4" className="px-6 py-12 text-center text-secondary/60 text-sm">No sold products in this period yet.</td>
 </tr>
 )}
 </tbody>
 </table>
 ...
 <div className="bg-utility-gray border border-utility-gray/60 rounded-2xl overflow-hidden backdrop-blur-sm">
 <div className="p-6 border-b border-utility-gray/60 flex items-center justify-between">
 <h4 className="font-serif font-bold text-xl text-secondary">Stock Manager</h4>
 <span className="text-[10px] text-secondary/60">Website stock · {totalStock.toLocaleString()} pieces</span>
 </div>
 ...
 className="w-full p-5 hover:bg-utility-gray transition-colors text-left"
 >
 <div className="flex items-center justify-between gap-4">
 <div className="flex items-center gap-4 min-w-0">
 <div className="w-14 h-14 rounded-xl overflow-hidden bg-primary border border-utility-gray/60 shrink-0">
 {product.thumbnail ? (
 <img src={resolveDisplayImageUrl(product.thumbnail)} alt={product.name} className="w-full h-full object-cover" />
 ) : (
 <div className="w-full h-full flex items-center justify-center text-secondary/70">
 <Package size={20} />
 </div>
 )}
 </div>
 ...
 <span className="px-4 py-2 border border-utility-gray/60 rounded-xl text-[10px] font-black text-accent-500">
 Manage
 </span>
 </div>
 </button>
 ...
 {stockModalProduct && (
 <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-primary backdrop-blur-sm">
 <div className="bg-utility-gray border border-utility-gray/60 rounded-3xl p-6 w-full max-w-3xl max-h-[88vh] overflow-y-auto custom-scrollbar shadow-2xl">
 <div className="flex items-start justify-between gap-4 mb-6">
 <div className="flex items-center gap-4 min-w-0">
 <div className="w-20 h-20 rounded-2xl overflow-hidden bg-primary border border-utility-gray/60 shrink-0">
 ...
 <div>
 <p className="text-xl font-serif font-bold text-secondary ">{stockModalProduct.name}</p>
 ...
 <button type="button" onClick={() => setStockModalProduct(null)} className="text-secondary/60 hover:text-accent-500">
 <X size={22} />
 </button>
 </div>
 ...
 <div key={stockKey(stockModalProduct, variant)} className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-center bg-primary border border-accent-500/5 rounded-2xl p-4">
 <div>
 <p className="text-[11px] font-black text-secondary">
 Size {variant.size || 'Standard'}{variant.color ? ` - ${variant.color}` : ''}
 </p>
 <p className="text-[9px] text-secondary/70 mt-1">
 ...
 className="w-10 h-10 rounded-xl bg-utility-gray border border-utility-gray/60 text-accent-500 font-black"
 >
 -
 </button>
 <input
 ...
 className="w-24 bg-utility-gray border border-utility-gray/60 rounded-xl py-3 px-3 text-secondary text-sm outline-none focus:border-accent-500/40 font-bold text-center"
 />
 <button
 type="button"
 onClick={() => setStockDrafts({
 ...
 <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-center bg-primary border border-accent-500/5 rounded-2xl p-4">
 <div>
 <p className="text-[11px] font-black text-secondary">Product total stock</p>
 <p className="text-[9px] text-secondary/70 mt-1">No size variants saved</p>
 </div>
 <input
 type="number"
 min="0"
 ...
 className="w-28 bg-utility-gray border border-utility-gray/60 rounded-xl py-3 px-3 text-secondary text-sm outline-none focus:border-accent-500/40 font-bold text-center"
 />
 </div>
 )}
 </div>

 <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-utility-gray/60">
 <button type="button" onClick={() => setStockModalProduct(null)} className="px-6 py-3 rounded-xl bg-utility-gray text-accent text-[10px] font-black ">
 Cancel
 </button>
 <button
 type="button"
 ...
 <div className="bg-utility-gray border border-utility-gray/60 rounded-3xl p-6 w-full max-w-3xl max-h-[88vh] overflow-y-auto custom-scrollbar shadow-2xl">
 <div className="flex items-center justify-between mb-6">
 <h4 className="text-2xl font-serif font-bold text-secondary ">Low Stock Sizes</h4>
 <button type="button" onClick={() => setIsLowStockOpen(false)} className="text-secondary/60 hover:text-accent-500">
 <X size={22} />
 </button>
 </div>
 <div className="space-y-3">
 ...
 className="w-full flex items-center justify-between gap-4 bg-primary border border-accent-500/5 rounded-2xl p-4 text-left hover:border-accent-500/30"
 >
 <div className="flex items-center gap-4 min-w-0">
 <div className="w-14 h-14 rounded-xl overflow-hidden bg-primary border border-utility-gray/60 shrink-0">
 ...
 <div>
 <p className="text-sm font-bold text-secondary ">{row.name}</p>
 <p className="text-[10px] text-secondary/60">
 Size {row.size}{row.color ? ` - ${row.color}` : ''} - {row.stockId || 'No stock ID'}
 </p>
 </div>
 </div>
 <span className="text-red-600 text-sm font-black">{row.stock} left</span>
 </button>
 )) : (
 <div className="py-12 text-center text-secondary/60 text-sm">No low stock sizes right now.</div>
 )}
 </div>
 </div>
 </div>
