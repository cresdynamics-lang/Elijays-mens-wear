...
const guessCategory = (name) => {
  ...
  if (n.includes('shoe') || n.includes('loafer') || n.includes('boot') || n.includes('sneaker')) return null;
  if (n.includes('belt') || n.includes('tie')) return 'Belts & Ties';
  if (n.includes('cap') || n.includes('hat')) return 'Caps & Hats';
  if (n.includes('suit')) return 'Suits';
  if (n.includes('jacket')) return 'Jackets';
  if (n.includes('polo')) return 'Polo T-shirts';
  if (n.includes('shirt') || n.includes('floral')) return 'Shirts';
  if (n.includes('khaki') || n.includes('trouser') || n.includes('jeans')) return 'Trousers';
  if (n.includes('sweater') || n.includes('sweat')) return 'Sweaters';
  if (n.includes('linen') || n.includes('linn')) return 'Linen';
  if (n.includes('vest') || n.includes('blazer')) return 'Blazers';
  if (['sock', 'boxer', 'scarf', 'hankie', 'hanky'].some((k) => n.includes(k))) return 'Accessories';
  return 'Apparel';
};
...
const importStockRows = async (
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!category) {
        results.warnings.push(`Skipped footwear line: ${row.name}`);
        continue;
      }
      ...
      if (!productId) {
        ...
        const ins = await client.query(
          ...
          [row.name, sku, category, shopPrice, onlinePrice]
        );
