// NEW — Parse ELIJAY'S Men's Wear stock Excel (Item, Opening, sales, Stock Out, Stock In, Closing)
...
const HEADER_MAP = {
  ...
  'retail price': 'shopPrice',
  'sell price': 'shopPrice',
  'pos price': 'shopPrice',
...
const parseCatalogWorksheet = (ws) => {
  ...
  for (let r = 2; r <= ws.rowCount; r++) {
    ...
    rows.push({
      ...
      storePrice: cols.storePrice ? num(row.getCell(cols.storePrice).value) : 0,
