...
const buildDescription = (meta) => {
  ...
  const intro = `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${meta.pattern !== 'solid' ? meta.pattern + ' ' : ''}${styleLabel} two-piece suit — tailored for weddings, galas, and executive occasions with premium structure and refined Kenyan style.`;
  const features = [
