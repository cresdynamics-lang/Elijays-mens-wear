/**
 * Import sweatshirts from backend/data/sweatshirts.json
 */
...
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'sweatshirts-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-sweatshirts-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const tshirtsParentId = await ensureCategory('T-shirts');
  const sweatshirtsId = await ensureCategory('Sweat-shirts', tshirtsParentId);
  ...
  for (const item of products) {
    ...
    console.log(`[Sweatshirt] ${item.name}`);
  ...
  console.log(`Done. Imported/updated ${imported} sweatshirts (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-sweatshirts-catalog failed:', err);
  process.exit(1);
});
