...
const buildDescription = (meta) => {
  const styleLabel =
    meta.style === 'double-breasted'
      ...
      : meta.style === 'peak-lapel'
        ? 'peak-lapel'
        : 'single-breasted';
  const intro = `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${meta.pattern !== 'solid' ? meta.pattern + ' ' : ''}${styleLabel} three-piece suit — a complete jacket, waistcoat, and trouser ensemble tailored for weddings, galas, and executive occasions.`;
  const features = [
  ...
  return `${intro}\n\nKey features:\n${features.map((f) => `• ${f}`).join('\n')}\n\nAvailable sizes S–3XL (jacket, waistcoat & trouser set). Tailored at ELIJAY'S Men's Wear for weddings, galas, and executive occasions.`;
};
