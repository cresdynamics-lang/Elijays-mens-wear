...
const CATALOG = [
  ...
  },
  {
    ...
    featured: true,
    description:
      'Rich purple classic knitted polo with smooth fine-gauge texture and three-button placket. Statement colour with understated ELIJAY'S Men's Wear finishing for events and casual Fridays.',
  },
  {
...
async function ensureKnittedPolosCategory() {
  ...
  const r = await db.query(
    `INSERT INTO categories (name, slug, description, parent_id)
     VALUES ('Knitted Polos', 'knitted-polos', 'Knitted Polos — ELIJAY'S Men's Wear', $1)
     RETURNING id`,
    [parentId]
  );
  return r.rows[0].id;
