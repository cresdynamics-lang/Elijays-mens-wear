...
const buildDescription = (meta) => {
  const intro =
    meta.subCategory === 'Formal shirts'
      ? `Elevate your formal rotation with this ${meta.color.toLowerCase()} ${meta.pattern} ${meta.sleeve}-sleeve shirt from ELIJAY'S Men's Wear. Cut for a sharp, confident presence, it bridges boardroom polish and celebration-day style without feeling stiff.`
      : `A standout ${meta.color.toLowerCase()} ${meta.pattern} ${meta.sleeve}-sleeve casual shirt curated for ELIJAY'S Men's Wear. Easy to style and comfortable in Kenya's climate, it delivers personality without sacrificing wearability.`;
