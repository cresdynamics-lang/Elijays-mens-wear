/**
 * Import two-piece suits from backend/data/two-piece-suits.json
 *
 * Usage:
 *   node scripts/generate-suit-specs.js
 *   node scripts/build-suits-catalog.js
 *   node scripts/import-suits-catalog.js
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'two-piece-suits.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'suit-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-suits-catalog.js');
    process.exit(1);
  }
  ...
  for (const item of products) {
    ...
    console.log(`[Two piece] ${item.name}`);
  ...
  console.log(`Done. Imported/updated ${imported} two-piece suits (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-suits-catalog failed:', err);
  process.exit(1);
});
