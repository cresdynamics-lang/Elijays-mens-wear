/**
 * Import formal & casual shirts from backend/data/formal-casual-shirts.json
 *
 * Usage:
 *   node scripts/build-shirts-catalog.js   # refresh catalog + copy images
 *   node scripts/import-shirts-catalog.js
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'formal-casual-shirts.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'shirt-images');
...
async function ensureCategory(name, parentId = null) {
  ...
  const result = await db.query(
    ...
    [name, slug, `${name} — ELIJAY'S Men's Wear`, parentId]
  );
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-shirts-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const shirtsParentId = await ensureCategory('Shirts');
  const formalId = await ensureCategory('Formal shirts', shirtsParentId);
  const casualId = await ensureCategory('Casual', shirtsParentId);
  ...
  for (const item of products) {
    ...
    const categoryId = item.subCategory === 'Formal shirts' ? formalId : casualId;
  ...
  console.log(`Done. Imported/updated ${imported} shirts (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-shirts-catalog failed:', err);
  process.exit(1);
});
