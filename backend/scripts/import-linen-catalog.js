/**
 * Import linen shirts from backend/data/linen.json
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'linen.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'linen-images');
...
async function ensureLinenShirtsCategory() {
  const found = await db.query("SELECT id FROM categories WHERE slug = 'linen-shirts'");
  if (found.rows.length) return found.rows[0].id;

  const linenParentId = await ensureCategory('Linen');
  return ensureCategory('Linen shirts', linenParentId);
}
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-linen-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const categoryId = await ensureLinenShirtsCategory();
  ...
  console.log(`Done. Imported/updated ${imported} linen shirts (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-linen-catalog failed:', err);
  process.exit(1);
});
