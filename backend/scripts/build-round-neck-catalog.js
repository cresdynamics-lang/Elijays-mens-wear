...
const ASSETS_DIR = path.join(
  ...
  '.cursor',
  'projects',
  'c-Users-Spine-Elijays-Mens-Wear',
  'assets'
);
...
const rich = (intro, features, fit, sizes = 'M–3XL') =>
  [
    ...
    '',
    `Available sizes ${sizes}. Exclusively at ELIJAY'S Men's Wear.`,
  ].join('\n');
...
const buildDescription = (meta, label) => {
  ...
  const intro =
    style.includes('graphic')
      ? `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} round-neck T-shirt with a bold chest graphic — soft cotton jersey built for everyday streetwear and casual comfort.`
      : style.includes('vintage')
        ? `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} vintage-wash round-neck tee — distressed finish and heavyweight cotton for an authentic lived-in streetwear look.`
        : style.includes('oversized')
          ? `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} oversized round-neck T-shirt — relaxed drop-shoulder fit and premium cotton jersey for modern casual styling.`
          : `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} solid round-neck essential tee — breathable cotton jersey designed as a versatile everyday staple.`;
