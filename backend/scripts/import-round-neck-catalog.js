/**
 * Import round-neck T-shirts from backend/data/round-neck.json
 */
...
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'round-neck-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-round-neck-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const tshirtsParentId = await ensureCategory('T-shirts');
  const roundNeckId = await ensureCategory('Round-neck T-shirts', tshirtsParentId);
  ...
  for (const item of products) {
    ...
    console.log(`[Round-neck] ${item.name}`);
  ...
  console.log(`Done. Imported/updated ${imported} round-neck T-shirts (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-round-neck-catalog failed:', err);
  process.exit(1);
});
