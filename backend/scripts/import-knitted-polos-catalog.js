/**
 * Import knitted polos from backend/data/knitted-polos.json
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'knitted-polos.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'knitted-polos-images');
...
async function ensureKnittedPolosCategory() {
  ...
  return ensureCategory('Knitted Polos', poloParentId);
}
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-knitted-polos-catalog.js');
    process.exit(1);
  }
  ...
  for (const item of products) {
    ...
    console.log(`[Knitted Polo] ${item.name}`);
  ...
  console.log(`Done. Imported/updated ${imported} knitted polos (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-knitted-polos-catalog failed:', err);
  process.exit(1);
});
