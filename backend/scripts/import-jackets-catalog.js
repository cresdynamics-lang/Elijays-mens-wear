/**
 * Import jackets from backend/data/jackets.json
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'jackets.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'jackets-images');
...
async function ensureJacketsCategory() {
  const found = await db.query("SELECT id FROM categories WHERE slug = 'jackets'");
  if (found.rows.length) return found.rows[0].id;
  return ensureCategory('Jackets');
}
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-jackets-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const categoryId = await ensureJacketsCategory();
  ...
  for (const item of products) {
    ...
    console.log(`[Jacket] ${item.name} — KSh ${item.price}`);
  ...
  console.log(`Done. Imported/updated ${imported} jackets (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-jackets-catalog failed:', err);
  process.exit(1);
});
