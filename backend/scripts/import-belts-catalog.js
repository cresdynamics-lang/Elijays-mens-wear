/**
 * Import belts from backend/data/belts.json
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'belts.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'belts-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-belts-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`Done. Imported/updated ${imported} belts (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-belts-catalog failed:', err);
  process.exit(1);
});
