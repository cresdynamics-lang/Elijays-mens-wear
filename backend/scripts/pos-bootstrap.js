...
const removeDemoSeller = async () => {
  const demoEmail = 'seller@elijays.co.ke';
...
const purgeTestSales = async () => {
  const demo = await db.query(
    `SELECT id FROM pos_profiles WHERE email = 'seller@elijays.co.ke' OR full_name ILIKE 'demo seller'`
  );
...
const seedSettings = async () => {
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@elijays.co.ke';
  await db.query(
