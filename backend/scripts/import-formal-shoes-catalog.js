/**
 * Import formal shoes from backend/data/formal-shoes.json
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'formal-shoes.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'formal-shoes-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-formal-shoes-catalog.js');
    process.exit(1);
  }
  ...
  for (const item of products) {
    ...
    console.log(`[Formal shoes] ${item.name}`);
  ...
  const repriced = await db.query(
    `UPDATE products SET price = CASE
       WHEN name ~* '(chelsea|boot)' THEN 7000
       ELSE 6500
     END
  ...
  console.log(`Done. Imported/updated ${imported} formal shoes (${skipped} skipped).`);
  console.log(`Repriced ${repriced.rowCount} products in Formal shoes category (boots 7000, officials 6500).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-formal-shoes-catalog failed:', err);
  process.exit(1);
});
