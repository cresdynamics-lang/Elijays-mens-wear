/**
 * Import three-piece suits from backend/data/three-piece-suits.json
 *
 * Usage:
 *   node scripts/generate-three-piece-specs.js
 *   node scripts/build-three-piece-catalog.js
 *   node scripts/import-three-piece-catalog.js
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'three-piece-suits.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'three-piece-suit-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-three-piece-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const suitsParentId = await ensureCategory('Suits');
  const threePieceId = await ensureCategory('Three piece', suitsParentId);
  ...
  for (const item of products) {
    ...
    console.log(`[Three piece] ${item.name}`);
  ...
  console.log(`Done. Imported/updated ${imported} three-piece suits (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-three-piece-catalog failed:', err);
  process.exit(1);
});
