...
const categoryTemplate = (cat, parent, name, color, brand) => {
  ...
  if (c.includes('sweat') || n.includes('sweatshirt') || n.includes('crewneck')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} crewneck sweatshirt — premium fleece pullover designed for warmth, comfort, and effortless casual style. ${brand ? `Featuring ${brand} quality and branding.` : 'Crafted for everyday wear with a relaxed streetwear appeal.'}`,
      features: [
      ...
      closing: 'Available sizes M–3XL. Exclusively at ELIJAY'S Men's Wear.',
    };
  }

  if (c.includes('round-neck') || c.includes('t-shirt') || c.includes('t-shirts') || n.includes('t-shirt') || n.includes('round-neck') || n.includes('tee')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} round-neck T-shirt — soft cotton jersey essential built for breathable comfort and everyday versatility. ${brand ? `${brand} athletic heritage meets casual style.` : 'A wardrobe staple for casual, streetwear, and weekend looks.'}`,
      features: [
  ...
  if (c.includes('loafer') || n.includes('loafer')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} leather loafers — refined slip-on footwear combining polished elegance with all-day comfort. ${brand ? `${brand} craftsmanship for discerning gentlemen.` : 'Perfect for business casual, events, and smart everyday dressing.'}`,
      features: [
      ...
      closing: 'Available EU sizes 40–46. Exclusively at ELIJAY'S Men's Wear.',
    };
  }

  if (c.includes('casual') && (c.includes('shoe') || n.includes('sneaker') || n.includes('trainer'))) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} casual sneakers — versatile low-top trainers designed for comfort, style, and daily wear. ${brand ? `${brand} sport-inspired design for modern gentlemen.` : 'From city streets to weekend outings.'}`,
      features: [
  ...
  if (c.includes('formal shoe') || n.includes('oxford') || n.includes('derby') || n.includes('brogue')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} formal dress shoes — expertly crafted leather footwear for business, weddings, and black-tie occasions.`,
      features: [
  ...
  if (c.includes('shirt') || n.includes('shirt')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} shirt — tailored menswear piece combining refined fabric with comfortable everyday wearability.`,
      features: [
  ...
  if (c.includes('suit') || n.includes('suit') || n.includes('blazer')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} suit — impeccably tailored menswear for formal occasions, business, and distinguished events.`,
      features: [
      ...
      closing: 'Available in standard and custom sizing. Exclusively at ELIJAY'S Men's Wear.',
    };
  }

  if (c.includes('trouser') || c.includes('khaki') || c.includes('chino') || c.includes('jean') || n.includes('trouser') || n.includes('jean')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} trousers — precision-cut bottoms designed for comfort, structure, and versatile styling.`,
      features: [
      ...
      closing: 'Available sizes 30–40 waist. Exclusively at ELIJAY'S Men's Wear.',
    };
  }

  if (c.includes('polo') || n.includes('polo')) {
    return {
      intro: `ELIJAY'S Men's Wear ${col} polo shirt — classic collared knit combining sporty heritage with smart-casual versatility.`,
      features: [
  ...
  return {
    intro: `ELIJAY'S Men's Wear ${name} — premium menswear crafted for quality, comfort, and distinguished style.`,
    features: [
      ...
      'Refined finish befitting the ELIJAY'S Men's Wear standard.',
      'Comfortable fit designed for the modern gentleman.',
      brand ? `${brand} quality you can trust.` : 'Exclusively curated for our clientele.',
    ],
    fit: 'Style according to occasion — pair with complementary pieces from the ELIJAY'S Men's Wear collection.',
    care: 'Follow care label instructions. Store properly to maintain shape and appearance.',
    closing: 'Available at ELIJAY'S Men's Wear — Kenya\'s destination for premium menswear.',
  };
};
