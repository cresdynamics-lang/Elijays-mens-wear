/**
 * Import jeans from backend/data/jeans.json
 *
 * Usage:
 *   node scripts/generate-jeans-specs.js
 *   node scripts/build-jeans-catalog.js
 *   node scripts/import-jeans-catalog.js
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'jeans.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'jeans-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-jeans-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const trousersParentId = await ensureCategory('Trousers');
  const jeansId = await ensureCategory('Jeans', trousersParentId);
  ...
  console.log(`Done. Imported/updated ${imported} jeans (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-jeans-catalog failed:', err);
  process.exit(1);
});
