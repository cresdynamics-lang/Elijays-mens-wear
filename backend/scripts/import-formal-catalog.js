/**
 * Import formal trousers from backend/data/formal-trousers.json
 *
 * Usage:
 *   node scripts/generate-formal-specs.js
 *   node scripts/build-formal-catalog.js
 *   node scripts/import-formal-catalog.js
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'formal-trousers.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'formal-trouser-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-formal-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const trousersParentId = await ensureCategory('Trousers');
  const formalId = await ensureCategory('Formal', trousersParentId);
  ...
  console.log(`Done. Imported/updated ${imported} formal trousers (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-formal-catalog failed:', err);
  process.exit(1);
});
