/**
 * Import casual shoes from backend/data/casual-shoes.json
 */
...
const CATALOG_PATH = path.join(__dirname, '..', 'data', 'casual-shoes.json');
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'casual-shoes-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-casual-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`Done. Imported/updated ${imported} casual shoes (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-casual-catalog failed:', err);
  process.exit(1);
});
