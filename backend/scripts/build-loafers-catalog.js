...
const buildDescription = (meta) => {
  const intro = `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${meta.style.replace(/-/g, ' ')} loafers — slip-on elegance for smart-casual and formal wear.`;
  const features = [
  ...
  return `${intro}\n\nKey features:\n${features.map((f) => `• ${f}`).join('\n')}\n\nAvailable EU sizes 40–46. Styled at ELIJAY'S Men's Wear for the modern gentleman.`;
};
