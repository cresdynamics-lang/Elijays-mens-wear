...
const buildDescription = (meta) => {
  const intro = `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${meta.style.replace(/-/g, ' ')} denim jeans — ${meta.fit} fit cotton denim for casual and smart-casual wear.`;
  const features = [
  ...
  return `${intro}\n\nKey features:\n${features.map((f) => `• ${f}`).join('\n')}\n\nAvailable waist sizes 30–40. Styled at ELIJAY'S Men's Wear for weekend, casual, and smart-casual wear.`;
};
