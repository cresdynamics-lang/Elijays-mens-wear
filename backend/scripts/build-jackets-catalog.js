/**
 * Build jackets catalog from Cursor assets batch.
 */
const fs = require('fs');
...
const OUT_DIR = path.join(__dirname, '..', 'data', 'jackets-images');
const OUT_JSON = path.join(__dirname, '..', 'data', 'jackets.json');
const SPECS_PATH = path.join(__dirname, '..', 'data', 'jackets-specs.json');
...
const main = () => {
  ...
  if (!Object.keys(SPECS).length) {
    console.error('Missing specs. Run: node scripts/generate-jackets-specs.js');
    process.exit(1);
  }
  ...
  for (const sourceFile of files) {
    ...
    idx += 1;
    const shortImage = `jacket-${String(idx).padStart(3, '0')}.png`;
  ...
  console.log(`Catalog: ${catalog.length} jackets`);
