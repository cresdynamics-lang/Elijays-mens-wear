...
const buildDescription = (meta) => {
  const intro = `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} crewneck sweatshirt — comfortable fleece pullover for casual and streetwear styling.`;
  const features = [
  ...
  return `${intro}\n\nKey features:\n${features.map((f) => `• ${f}`).join('\n')}\n\nAvailable sizes M–3XL. Styled at ELIJAY'S Men's Wear.`;
};
