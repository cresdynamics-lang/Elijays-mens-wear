...
const buildDescription = (meta) => {
  const intro = `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${meta.style.replace(/-/g, ' ')} formal trousers — ${meta.fit} fit suiting for boardrooms, weddings, and black-tie occasions.`;
  const features = [
  ...
  return `${intro}\n\nKey features:\n${features.map((f) => `• ${f}`).join('\n')}\n\nAvailable waist sizes 30–40. Tailored at ELIJAY'S Men's Wear for boardrooms, weddings, and formal occasions.`;
};
