/**
 * Import caps & hats from backend/data/caps-hats.json
 */
...
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'caps-hats-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-caps-hats-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const capsHatsId = await ensureCategory('Caps & Hats');
  ...
  for (const item of products) {
    ...
    console.log(`[Caps & Hats] ${item.name}`);
    ...
    const variant = { size: SIZE, color: item.color, image_url: imageUrl };
  ...
  console.log(`Done. Imported/updated ${imported} caps & hats (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-caps-hats-catalog failed:', err);
  process.exit(1);
});
