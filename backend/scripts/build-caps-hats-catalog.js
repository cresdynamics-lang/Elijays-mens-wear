...
const rich = (intro, features, fit, sizes = 'One Size (adjustable)') => {
  const text = [
    ...
    '',
    `${sizes}. Exclusively at ELIJAY'S Men's Wear.`,
  ].join('\n');
...
const buildDescription = (meta, label) => {
  ...
  const intro =
    hatType.includes('fedora') || hatType.includes('trilby') || hatType.includes('panama') || hatType.includes('porkpie')
      ? `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${hatType} — refined headwear combining classic silhouette with premium materials for distinguished casual and event styling.`
      : hatType.includes('trucker')
        ? `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} trucker hat — mesh-back casual cap with graphic front panel for streetwear and outdoor style.`
        : hatType.includes('performance') || hatType.includes('driver')
          ? `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} performance cap — lightweight breathable headwear built for sport, golf, and active casual wear.`
          : `ELIJAY'S Men's Wear ${meta.color.toLowerCase()} ${hatType} — quality cotton headwear with comfortable fit for everyday casual and streetwear looks.`;
