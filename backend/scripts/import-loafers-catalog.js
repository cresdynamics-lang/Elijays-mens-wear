/**
 * Import loafers from backend/data/loafers.json
 *
 * Usage:
 *   node scripts/generate-loafers-specs.js
 *   node scripts/build-loafers-catalog.js
 *   node scripts/import-loafers-catalog.js
 */
...
const IMAGES_DIR = path.join(__dirname, '..', 'data', 'loafers-images');
...
async function run() {
  if (!fs.existsSync(CATALOG_PATH)) {
    console.error('Missing catalog. Run: node scripts/build-loafers-catalog.js');
    process.exit(1);
  }

  const products = JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
  const shoesParentId = await ensureCategory('Shoes');
  const loafersId = await ensureCategory('Loafers', shoesParentId);
  ...
  console.log(`Done. Imported/updated ${imported} loafers (${skipped} skipped).`);
  process.exit(0);
}

run().catch((err) => {
  console.error('import-loafers-catalog failed:', err);
  process.exit(1);
});
