# Sync ELIJAY'S Men's Wear database (local → Ubuntu server)
...
Database name used in examples: `eljays_db` (match your `.env`).
...
```powershell
cd C:\Users\HomePC\Desktop\Cres\Elijays-Mens-Wear\backend

# Set variables (adjust password/host as in your .env)
$env:PGPASSWORD = "YOUR_LOCAL_DB_PASSWORD"

pg_dump -h localhost -p 5432 -U postgres -d eljays_db -F c -f eljays_db_backup.dump
```

Plain SQL format (alternative):

```powershell
pg_dump -h localhost -p 5432 -U postgres -d eljays_db > eljays_db_backup.sql
```

### 2. Copy file to Ubuntu

```powershell
scp eljays_db_backup.dump user@YOUR_SERVER_IP:/home/user/
```
...
psql -h localhost -U postgres -c "CREATE DATABASE eljays_db;" 2>/dev/null || true

# Restore (custom format)
pg_restore -h localhost -U postgres -d eljays_db --clean --if-exists ~/eljays_db_backup.dump

# OR restore (SQL file)
# psql -h localhost -U postgres -d eljays_db < ~/eljays_db_backup.sql
```
...
Uploads use preset **`elijays-mens-wear`** and folder **`elijays-mens-wear`**.

In Cloudinary Dashboard → Settings → Upload → Upload presets:

1. Create or edit preset named exactly: `elijays-mens-wear`
...
CLOUDINARY_UPLOAD_PRESET=elijays-mens-wear
CLOUDINARY_FOLDER=elijays-mens-wear
```
...
```bash
psql -h localhost -U postgres -d eljays_db
```
